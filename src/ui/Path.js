...
function Path(props) {
	...
	return <div className='path_scroll'><div className='path_view_port'>{intersperse(pathList.map((pathItem, index) => <span key={index}>{pathItem}</span>), <span className='path_separator'>/</span>)}</div></div>;
}
